...
# ================= ULTRA ANTI-LOGOUT V3 - 48HR+ SESSION FIX =================
def session_keepalive_loop(uid: str):
    """Keeps session alive 48hr+ - Random intervals + Real user activity simulation"""
    uid = str(uid)
    while True:
        try:
            # Random 15-35 min - human-like, not bot pattern
            sleep_time = random.randint(15*60, 35*60)
            time.sleep(sleep_time)
            
            if uid not in bot_running or not bot_running.get(uid):
                continue
                
            sess = account_sessions.get(uid) or get_account_session(uid)
            if not sess:
                continue
                
            session_obj = sess.get("session")
            headers = sess.get("headers")
            if not session_obj or not headers:
                continue
                
            try:
                # Rotate through REAL Instagram app endpoints - not just one
                endpoints = [
                    f"{API_BASE_URL}/news/inbox/",
                    f"{API_BASE_URL}/accounts/current_user/?edit=true",
                    f"{API_BASE_URL}/feed/timeline/",
                    f"{API_BASE_URL}/direct_v2/inbox/?visual_message_return_type=unseen&thread_message_limit=5&persistentBadging=true",
                    f"{API_BASE_URL}/accounts/get_presence_disabled/",
                ]
                url = random.choice(endpoints)
                
                r = session_obj.get(url, headers=headers, timeout=15)
                
                if r.status_code == 200:
                    log("INFO", f"Keepalive OK 200 for @{sess.get('username')} - Session Extended")
                    # CRITICAL: Save updated cookies from response
                    try:
                        for cookie in session_obj.cookies:
                            if cookie.name in ["csrftoken", "rur", "mid", "ig_did"]:
                                # Update stored session in db
                                db = load_db()
                                if uid in db.get("accounts", {}):
                                    if cookie.name == "csrftoken":
                                        db["accounts"][uid]["csrf_token"] = cookie.value
                                    save_db(db)
                        
                        # Update CSRF header
                        csrf = get_cookie(session_obj, "csrftoken", "")
                        if csrf:
                    ...
                    log("WARN", f"Keepalive got {r.status_code} for @{sess.get('username')} - IG Rate limit, backing off 10min")
                    # Back off - don't spam keepalive when flagged
                    time.sleep(600)
                    continue
                else:
                    log("WARN", f"Keepalive got {r.status_code} for @{sess.get('username')}")
                    
            except Exception as e:
                log("WARN", f"Keepalive failed for {uid}: {e}")
                
        except Exception as e:
            time.sleep(60)

def start_keepalive(uid: str):
    th = threading.Thread(target=session_keepalive_loop, args=(uid,), daemon=True)
    th.start()
    return th

# Store keepalive threads
keepalive_threads = {}
...
def generate_device_profile(username: str = "") -> dict:
    """ FIXED: Persistent device per username - prevents logout due to device change """
    uname = (username or "instagram_user").strip().lower()
    # Persistent storage
    try:
        os.makedirs("device_cache", exist_ok=True)
        cache_file = os.path.join("device_cache", f"{uname}_device.json")
...
def verify_session(session: requests.Session, headers: dict) -> dict:
    """
    FIXED: Single mobile verification only - Web inbox check was causing Chrome logout
    """
    # Only use mobile endpoint to avoid triggering Instagram security logout
    try:
    ...
    # Fallback: try inbox but with mobile headers only
    try:
        inbox_url = "https://i.instagram.com/api/v1/direct_v2/inbox/?persistentBadging=true&limit=5"
        r = session.get(inbox_url, headers=headers, timeout=12)
        if r.status_code == 200:
            data = r.json()
            if data.get("inbox", {}).get("viewer"):
                viewer = data["inbox"]["viewer"]
                if viewer.get("username"):
                    return viewer
    except Exception:
        pass
    return {}
...
def create_instagram_session(username: str, password: str) -> tuple[bool, dict, str]:
    ...
    try:
        ...
        if data.get("authenticated") is True:
            ...
            identifier = str(info.get("two_factor_identifier") or "")
            if identifier:
                challenge_token = uuid.uuid4().hex
                with pending_2fa_lock:
                    pending_2fa[challenge_token] = {
                        "session": session, "username": username.strip(),
                        "identifier": identifier,
                        "csrf_token": session.cookies.get("csrftoken", csrf_token),
                        "device_id": device_id, "android_id": android_id,
                        "created": time.time(),
                    }
                return False, {"two_factor_required": True, "challenge_token": challenge_token}, "2FA verification required."
            return False, {}, "Instagram requested 2FA, but no verification session was returned. Please retry."
...
app = Flask(__name__)
app.secret_key = "ULTRA_PANEL_KEY_KINGX_SCAR_CLAN_FINAL_V3_48HR"
...
<title>Install SINISTERS CLAN on iPhone</title>
<style>
...
<div class="card">
  <div class="icon-box">
    <img src="/static/logo.png" alt="SINISTERS CLAN">
  </div>
...
<dict>
    ...
    <array>
        <dict>
            ...
            <true/>
            <key>Label</key>
            <string>SINISTERS CLAN</string>
            <key>PayloadDescription</key>
            <string>SINISTERS CLAN Autonomous Instagram Command Matrix</string>
            <key>PayloadDisplayName</key>
            <string>SINISTERS CLAN</string>
            <key>PayloadIdentifier</key>
            <string>com.SINISTERS CLAN.webclip</string>
            <key>PayloadType</key>
    ...
    <string>SINISTERS CLAN</string>
    <key>PayloadIdentifier</key>
    <string>com.SINISTERS CLAN.profile</string>
...
def api_downloads_info():
    ...
    return jsonify({
        ...
        "ios": {
            "name": "SINISTERS CLAN APPLE iOS APP",
        ...
        "iso": {
            "name": "SINISTERS CLAN HYBRID ISO SUITE",
...
def api_global_settings():
    ...
    if request.method == "POST":
        ...
        self_ping_interval = float(data.get("self_ping_interval", data.get("SELF_PING_INTERVAL", 300)))
        if self_ping_interval < 30:
            self_ping_interval = 30  # Minimum 30s to avoid hammering
        ...
        g_cfg["self_ping_interval"] = self_ping_interval
    ...
    return jsonify({
        ...
        "self_ping_interval": g_cfg.get("self_ping_interval", float(os.getenv("SELF_PING_INTERVAL", "300"))),
...
@app.route("/api/accounts/login/verify", methods=["POST"])
def api_verify_account_2fa():
    if "role" not in session:
        return jsonify({"success": False, "error": "Login required"}), 401

    data = request.json or {}
    token = str(data.get("challenge_token") or "").strip()
    code = str(data.get("verification_code") or "").strip()
    if not token or not code:
        return jsonify({"success": False, "error": "Verification code is required."}), 400

    with pending_2fa_lock:
        pending = pending_2fa.get(token)
    if not pending:
        return jsonify({"success": False, "error": "Verification session expired. Start the login again."}), 400
    if time.time() - float(pending.get("created", 0)) > 600:
        with pending_2fa_lock:
            pending_2fa.pop(token, None)
        return jsonify({"success": False, "error": "Verification session expired. Start the login again."}), 400

    sess = pending["session"]
    csrf_token = pending.get("csrf_token") or DEFAULT_CSRF_TOKEN
    headers = {
        "User-Agent": WEB_USER_AGENT,
        "X-IG-App-ID": IG_WEB_APP_ID,
        "X-Requested-With": "XMLHttpRequest",
    ...
    try:
        ...
        if not (result.get("authenticated") is True or sess.cookies.get("sessionid")):
            return jsonify({"success": False, "error": result.get("message") or "Invalid verification code or verification failed."}), 400

        sessionid = sess.cookies.get("sessionid", "")
        if not sessionid:
            return jsonify({"success": False, "error": "Instagram did not return a session after verification."}), 400

        csrf_token = sess.cookies.get("csrftoken", csrf_token)
        ds_user_id = str(result.get("userId") or sess.cookies.get("ds_user_id", ""))
        uid = next_uid(load_db())
        current_user = session.get("user", "OWNER")

        acc_session = init_account_session(uid, sessionid, csrf_token=csrf_token)
        uname = acc_session.get("username") or pending["username"]
        pk = acc_session.get("pk") or ds_user_id

        default_settings = {
            "target_name": "", "group_name_lock": "",
            "between_messages_sec": 35.0, "between_renames_sec": 120.0,
            "single_cycle_wait_sec": 80.0, "multi_cycle_wait_sec": 120.0,
            "cycle_wait_sec": 80.0, "texts_per_nc": 10,
            "rename_enabled": True, "rotation_mode": "sequential"
        }
        db = load_db()
        db["accounts"][uid] = {
            "sessionid": sessionid, "csrf_token": csrf_token, "username": uname,
            "user_id": pk, "prefix": "", "delay": 35, "owner": current_user,
            "mode": "multi_gc", "target_name": "", "group_name_lock": "",
            "single_thread_id": "", "single_group_title": "", "groups": [],
            "settings": default_settings
        }
        bot_running[uid] = False
        stats[uid] = {"user": current_user, "account": uid, "sent": 0, "failed": 0,
                      "renamed": 0, "rename_failed": 0, "running": False,
                      "started": None, "log": "Authenticated & Ready"}
        live_stats[uid] = {"running": False, "started": None}
        save_db(db)
        ...
        add_log("SUCCESS", f"Account added after 2FA verification: @{uname} (UID #{uid})", uname, "system")
        return jsonify({"success": True, "account": {"account_id": uid, "username": uname, "user_id": pk}})
    except Exception as e:
        return jsonify({"success": False, "error": f"Verification failed: {e}"}), 400
...
def api_add_account():
    ...
    # Username/password mode. The password is used only for authentication
    # and is never persisted in db.json or returned to the browser.
    if not sessionid and username and password:
        ok, auth_data, err = create_instagram_session(username, password)
        if not ok:
            if auth_data.get("two_factor_required") and auth_data.get("challenge_token"):
                return jsonify({
                    "success": False,
            ...
            return jsonify({"success": False, "error": err or "Instagram username/password login failed."}), 400
        sessionid = auth_data.get("session_id", "")
        csrf_token = auth_data.get("csrf_token", csrf_token)

    if not sessionid:
        return jsonify({"success": False, "error": "Enter a Session ID or Username + Password."})
...
def start_bot_worker(uid, mode=None):
    ...
    # FIXED: Start keepalive to prevent 10-15hr logout
    if uid not in keepalive_threads or not keepalive_threads[uid].is_alive():
        keepalive_threads[uid] = start_keepalive(uid)
        log("INFO", f"Anti-logout keepalive started for UID {uid}")
...
def start_cloudflared_tunnel(port=20823):
    ...
    while True:
        try:
            ...
            print(f"[*] 🚀 Launching Cloudflare Public Tunnel for SINISTERS CLAN on Port {port}...")
...
if __name__ == "__main__":
    ...
    print(f"============================================================")
    print(f"       ⚡ SINISTERS CLAN • AUTONOMOUS MATRIX ⚡          ")
    print(f"               OWNER KEY: SCAR@12345                        ")
    print(f"               RUNNING ON PORT: {port}                      ")
    print(f"               SELF_URL: {SELF_URL or '(not set)'}          ")
    print(f"               SELF_PING_INTERVAL: {SELF_PING_INTERVAL}s    ")
    print(f"============================================================")
