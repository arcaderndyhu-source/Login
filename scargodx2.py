...
# ================= FIXED: ANTI-LOGOUT KEEPALIVE SYSTEM =================
def session_keepalive_loop(uid: str):
    """Keeps session alive every 25 minutes to prevent 10-15hr expiry"""
    uid = str(uid)
    while True:
        try:
            time.sleep(25 * 60)  # 25 min
            ...
            if session and headers:
                # Light ping to keep session valid
                try:
                    url = f"{API_BASE_URL}/accounts/current_user/?edit=true"
                    r = session.get(url, headers=headers, timeout=10)
                    if r.status_code == 200:
                        log("INFO", f"Keepalive OK for @{sess.get('username')}")
                    else:
                        log("WARN", f"Keepalive got {r.status_code} for @{sess.get('username')} - session may need refresh")
